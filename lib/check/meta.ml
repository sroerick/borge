...
let sexp_of_documentation (doc : Doc_coverage.file_coverage) =
  let buf = Buffer.create 256 in
  Buffer.add_string buf "(documentation\n";
  Buffer.add_string buf (Printf.sprintf "  (file %s)\n" (sexp_of_string doc.path));
  ...
  if doc.undocumented_names <> [] then begin
    Buffer.add_string buf "  (undocumented-names\n";
    List.iter (fun name ->
      Buffer.add_string buf (Printf.sprintf "    (name %s)\n" (sexp_of_string name))
    ) doc.undocumented_names;
    Buffer.add_string buf "  ))\n"
  end;
  Buffer.add_string buf ")";
  Buffer.contents buf

let string_of_meta m ?(docs=None) () =
  ...
  (match docs with
   | Some doc ->
       Buffer.add_string buf "  ";
       Buffer.add_string buf (sexp_of_documentation doc);
       Buffer.add_char buf '\n';
   | None -> ());
  Buffer.add_string buf ")\n";
  Buffer.contents buf
...
let write_meta path m =
  let content = string_of_meta m ~docs:None () in
  let oc = open_out path in
  output_string oc content;
  close_out oc

(** Write a .borg.meta file with documentation coverage *)
let write_meta_with_docs path m (doc : Doc_coverage.file_coverage) =
  let content = string_of_meta m ~docs:(Some doc) () in
  let oc = open_out path in
  output_string oc content;
  close_out oc
