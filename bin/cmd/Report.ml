...
let run dir json quiet =
  ...
  Printf.printf "%s  implemented  verified  in-progress  planned  deprecated\n" (pad_right max_name "file");
  ...
  List.iter (fun (r : Report.file_stats) ->
    let display = match r.project_name with
      ...
      | None -> Filename.basename r.path
    in
    Printf.printf "%s      %3d        %3d        %3d       %3d       %3d\n"
      (pad_right max_name display) r.implemented r.verified r.in_progress r.planned r.deprecated
  ) result.files;
  if result.files <> [] then (
    Printf.printf "%s\n" (String.make (max_name + 50) '-');
    Printf.printf "%s      %3d        %3d        %3d       %3d       %3d\n"
      (pad_right max_name "total") result.totals.implemented result.totals.verified result.totals.in_progress result.totals.planned result.totals.deprecated
  );
  let total = result.totals.implemented + result.totals.verified + result.totals.in_progress + result.totals.planned + result.totals.deprecated in
  Printf.printf "\n%d implemented | %d verified | %d in-progress | %d planned | %d deprecated | %d total tracked\n"
    result.totals.implemented result.totals.verified result.totals.in_progress result.totals.planned result.totals.deprecated total;
